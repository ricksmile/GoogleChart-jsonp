...
$(document).ready(function () {
    ...
    $('div.menu1').delegate('input', 'click', function () {
        ChartTitle=$(this).siblings('span').html();
...
function getGeoData(xmldata) {
    ...
    case 'Bar':
        //建立人口長條圖 chart1_div
        geoDataObj.createMutiBar(ChartTitle, '', geoTable, 'chart1_div');
        break;
...
function checkedResourse() {
    ...
    switch (CheckMode) {
        ...
        case 2: //carList;
            ...
            break;
        case 3: //imcomeList;
            $.each($('div.imcomeList').find('input'), function (i, field) {
                switch (field.checked) {
                    case true:
                        Resource_id = field.value;
                        ChartMode = $(field).attr('mode');
...
function queryGeoData() {
    ...
    var pa = {
        resource_id: Resource_id,
        'filters[station]': arrayStr
    };
    //如果沒出現限制欄位，全部秀出
    if (ColArray === null)
        delete pa['filters[station]'];
...
function formGeoData(xmldata) {
    ...
    switch (ChartMode) {
        ...
        case 'Bar':
            //建立人口長條圖 chart1_div
            geoDataObj.createMutiBar(ChartTitle, unit, geoTable, 'chart1_div');
            break;
