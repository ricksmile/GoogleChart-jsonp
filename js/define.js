...
CreateGeoDataObj = function() {
    ...
    this.createTable = function(geoTable,tableId) {
     ...
     this.createMap = function(geoView,mapId) {
      ...
      //建立多重折線圖
     this.createMutiLine = function(title,unit,geotable,chartId) {
      ...
      //var reginArray = geotable.getDistinctValues(0);
       ...
       //建立多重長條圖
     this.createMutiBar = function(title,unit,geotable,chartId) {
        ...
        if(NumberOfRows>6){
        $(document.getElementById(chartId)).html('請選擇縣市');
        return;
        }
